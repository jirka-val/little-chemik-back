...
def check_AMBER_topology(topology):
    checks = [
        (["ATOM_NAME", "CHARGE", "ATOMIC_NUMBER", "MASS", "ATOM_TYPE_INDEX", "NUMBER_EXCLUDED_ATOMS", "AMBER_ATOM_TYPE",
          "TREE_CHAIN_CLASSIFICATION", "JOIN_ARRAY", "IROTAT", "RADII", "SCREEN"], topology["POINTERS"][0], "atoms"),
        (["RESIDUE_LABEL", "RESIDUE_POINTER"], topology["POINTERS"][11], "residues"),
        (["NONBONDED_PARM_INDEX"], topology["POINTERS"][1] ** 2, "nonbond pairs"),
        (["LENNARD_JONES_ACOEF", "LENNARD_JONES_BCOEF"], topology["POINTERS"][1] * (topology["POINTERS"][1] + 1) // 2,
        ...
        (
        ["DIHEDRAL_FORCE_CONSTANT", "DIHEDRAL_PERIODICITY", "DIHEDRAL_PHASE", "SCEE_SCALE_FACTOR", "SCNB_SCALE_FACTOR"],
        topology["POINTERS"][17], "dihedral parameters"),
...
def add_atomic_nonbond_parm(topology):
    ...
    for i in range(ntypes):
        ...
        if topology["LENNARD_JONES_ACOEF"][nonbonded_diagonal_index[i]] == 0.0 or topology["LENNARD_JONES_BCOEF"][
            nonbonded_diagonal_index[i]] == 0.0:
            nonbonded_diagoval_R.append(0.0)
            nonbonded_diagoval_eps.append(0.0)
        else:
            nonbonded_diagoval_R.append(0.5 * (2 * topology["LENNARD_JONES_ACOEF"][nonbonded_diagonal_index[i]] /
                                               topology["LENNARD_JONES_BCOEF"][nonbonded_diagonal_index[i]]) ** (
                                                    1.0 / 6.0))
            nonbonded_diagoval_eps.append(
                0.25 * topology["LENNARD_JONES_BCOEF"][nonbonded_diagonal_index[i]] * topology["LENNARD_JONES_BCOEF"][
                    nonbonded_diagonal_index[i]] / topology["LENNARD_JONES_ACOEF"][nonbonded_diagonal_index[i]])
        ...
        for j in range(i):
            ...
            if index >= 0:
                if topology["LENNARD_JONES_ACOEF"][index] == 0.0 or topology["LENNARD_JONES_BCOEF"][index] == 0.0:
                    if nonbonded_diagoval_R[i] != 0.0 and nonbonded_diagoval_R[j] != 0.0 and nonbonded_diagoval_eps[
                        i] != 0.0 and nonbonded_diagoval_eps[j] != 0.0:
                        nonbonded_NBfix.append((i, j))
                else:
                    R = (2 * topology["LENNARD_JONES_ACOEF"][index] / topology["LENNARD_JONES_BCOEF"][index]) ** (
                                1.0 / 6.0)
                    eps = 0.25 * topology["LENNARD_JONES_BCOEF"][index] * topology["LENNARD_JONES_BCOEF"][index] / \
                          topology["LENNARD_JONES_ACOEF"][index]
                    if abs(R - nonbonded_diagoval_R[i] - nonbonded_diagoval_R[j]) > 0.0001 or abs(
                            math.sqrt(nonbonded_diagoval_eps[i] * nonbonded_diagoval_eps[j]) - eps) > 0.0001:
                        nonbonded_NBfix.append((i, j))
...
def strip(topology, ntwprt):
    ...
    for i in range(0, len(topology["BONDS_INC_HYDROGEN"]), 3):
        if topology["BONDS_INC_HYDROGEN"][i] < ind_lim and topology["BONDS_INC_HYDROGEN"][i + 1] < ind_lim:
            new_bnd_h.extend([topology["BONDS_INC_HYDROGEN"][i], topology["BONDS_INC_HYDROGEN"][i + 1],
                              topology["BONDS_INC_HYDROGEN"][i + 2]])
    ...
    for i in range(0, len(topology["BONDS_WITHOUT_HYDROGEN"]), 3):
        if topology["BONDS_WITHOUT_HYDROGEN"][i] < ind_lim and topology["BONDS_WITHOUT_HYDROGEN"][i + 1] < ind_lim:
            new_bnd_a.extend([topology["BONDS_WITHOUT_HYDROGEN"][i], topology["BONDS_WITHOUT_HYDROGEN"][i + 1],
                              topology["BONDS_WITHOUT_HYDROGEN"][i + 2]])
    ...
    new_ang_h = []
    for i in range(0, len(topology["ANGLES_INC_HYDROGEN"]), 4):
        if topology["ANGLES_INC_HYDROGEN"][i] < ind_lim and topology["ANGLES_INC_HYDROGEN"][i + 1] < ind_lim and \
                topology["ANGLES_INC_HYDROGEN"][i + 2] < ind_lim:
            new_ang_h.extend([topology["ANGLES_INC_HYDROGEN"][i], topology["ANGLES_INC_HYDROGEN"][i + 1],
                              topology["ANGLES_INC_HYDROGEN"][i + 2], topology["ANGLES_INC_HYDROGEN"][i + 3]])
    topology["ANGLES_INC_HYDROGEN"] = new_ang_h
    topology["POINTERS"][4] = len(topology["ANGLES_INC_HYDROGEN"]) // 4
    new_ang_a = []
    for i in range(0, len(topology["ANGLES_WITHOUT_HYDROGEN"]), 4):
        if topology["ANGLES_WITHOUT_HYDROGEN"][i] < ind_lim and topology["ANGLES_WITHOUT_HYDROGEN"][i + 1] < ind_lim and \
                topology["ANGLES_WITHOUT_HYDROGEN"][i + 2] < ind_lim:
            new_ang_a.extend([topology["ANGLES_WITHOUT_HYDROGEN"][i], topology["ANGLES_WITHOUT_HYDROGEN"][i + 1],
                              topology["ANGLES_WITHOUT_HYDROGEN"][i + 2], topology["ANGLES_WITHOUT_HYDROGEN"][i + 3]])
    ...
    new_dih_h = []
    for i in range(0, len(topology["DIHEDRALS_INC_HYDROGEN"]), 5):
        if topology["DIHEDRALS_INC_HYDROGEN"][i] < ind_lim and topology["DIHEDRALS_INC_HYDROGEN"][
            i + 1] < ind_lim and abs(topology["DIHEDRALS_INC_HYDROGEN"][i + 2]) < ind_lim and abs(
                topology["DIHEDRALS_INC_HYDROGEN"][i + 3]) < ind_lim:
            new_dih_h.extend([topology["DIHEDRALS_INC_HYDROGEN"][i], topology["DIHEDRALS_INC_HYDROGEN"][i + 1],
                              topology["DIHEDRALS_INC_HYDROGEN"][i + 2], topology["DIHEDRALS_INC_HYDROGEN"][i + 3],
                              topology["DIHEDRALS_INC_HYDROGEN"][i + 4]])
    topology["DIHEDRALS_INC_HYDROGEN"] = new_dih_h
    topology["POINTERS"][6] = len(topology["DIHEDRALS_INC_HYDROGEN"]) // 5
    new_dih_a = []
    for i in range(0, len(topology["DIHEDRALS_WITHOUT_HYDROGEN"]), 5):
        if topology["DIHEDRALS_WITHOUT_HYDROGEN"][i] < ind_lim and topology["DIHEDRALS_WITHOUT_HYDROGEN"][
            i + 1] < ind_lim and abs(topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 2]) < ind_lim and abs(
                topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 3]) < ind_lim:
            new_dih_a.extend([topology["DIHEDRALS_WITHOUT_HYDROGEN"][i], topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 1],
                              topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 2],
                              topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 3],
                              topology["DIHEDRALS_WITHOUT_HYDROGEN"][i + 4]])
    ...
    topology["POINTERS"][17] = max(
        topology["DIHEDRALS_INC_HYDROGEN"][4::5] + topology["DIHEDRALS_WITHOUT_HYDROGEN"][4::5])
...
def correct_VdWmatrix_by_molecule_types(topology):
    ...
    for i in range(ntyp):
        ...
        outside_js = [j for j in range(ntyp) if
                      j != i and any(mol_j not in mols_i for mol_j in topology["MOLECULE_TYPES_BY_ATOM_TYPES"][j])]
        if outside_js and all(M[i][j] == 1 or M[i][j] == 2 for j in outside_js) and not all(
                M[i][j] == 2 for j in range(ntyp)):
            candidates.append(i)
    ...
    for i in candidates:
        mols_i = topology["MOLECULE_TYPES_BY_ATOM_TYPES"][i]
        outside_js = [j for j in range(ntyp) if j != i and any(
            mol_j not in mols_i for mol_j in topology["MOLECULE_TYPES_BY_ATOM_TYPES"][j]) and j not in candidates and
                      M[i][j] != 2]
        R_can = []
        eps_can = []
        for j in outside_js:
        ...
        R_tol = max(R_can, default=topology["NONBONDED_DIAGONAL_R"][i]) - min(R_can,
                                                                              default=topology["NONBONDED_DIAGONAL_R"][
                                                                                  i])
        R_est = (max(R_can, default=topology["NONBONDED_DIAGONAL_R"][i]) + min(R_can,
                                                                               default=topology["NONBONDED_DIAGONAL_R"][
                                                                                   i])) / 2.0
        eps_tol = max(eps_can, default=topology["NONBONDED_DIAGONAL_EPS"][i]) - min(eps_can, default=
        topology["NONBONDED_DIAGONAL_EPS"][i])
        eps_est = (max(eps_can, default=topology["NONBONDED_DIAGONAL_EPS"][i]) + min(eps_can, default=
        topology["NONBONDED_DIAGONAL_EPS"][i])) / 2.0
        # print(R_can)
        # print(eps_can)
        ...
        for c in (nt for nt in neighbors if nt > b):
            for a in (nt for nt in neighbors if nt != c):
                for d in (nt for nt in connectivity[c] if nt != b and nt != a):
                    ...
                    key = min((new_types[a], new_types[b], new_types[c], new_types[d]),
                              (new_types[d], new_types[c], new_types[b], new_types[a]))
                    ...
                    for dihe_term in dihedral_p:
                        ...
                        if any(top['ATOMIC_NUMBER'][i] == 1 for i in [a, b, c, d]):
                            top['DIHEDRALS_INC_HYDROGEN'].extend(
                                [3 * a, 3 * b, 3 * c * pn_fact, 3 * d, dihedral_parms.index(dihe_term)])
                        else:
                            top['DIHEDRALS_WITHOUT_HYDROGEN'].extend(
                                [3 * a, 3 * b, 3 * c * pn_fact, 3 * d, dihedral_parms.index(dihe_term)])
                        pn_fact = -1
    for ic0, neighbors in connectivity.items():
        if mol_types[ic0].startswith('W'):
            continue
        ...
        if len(neighbors) == 3:  # impropers
            ...
            for dihe_term in dihedral_p:
                ...
                if any(top['ATOMIC_NUMBER'][i] == 1 for i in [ia, ib, ic, id]):
                    top['DIHEDRALS_INC_HYDROGEN'].extend(
                        [3 * ia, 3 * ib, -3 * ic, -3 * id, dihedral_parms.index(dihe_term)])
                else:
                    top['DIHEDRALS_WITHOUT_HYDROGEN'].extend(
                        [3 * ia, 3 * ib, -3 * ic, -3 * id, dihedral_parms.index(dihe_term)])
